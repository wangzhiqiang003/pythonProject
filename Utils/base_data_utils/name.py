...
class Name:
    ...
    def random_name():
        # 删减部分，比较大众化姓氏
        firstName = "赵钱孙李周吴郑王冯陈褚卫蒋沈韩杨朱秦尤许何吕施张孔曹严华金魏陶姜戚谢邹喻水云苏潘葛奚范彭郎鲁韦昌马苗凤花方俞任袁柳鲍史唐费岑薛雷贺倪汤滕殷罗毕郝邬安常乐于时傅卞齐康伍余元卜顾孟平"                 "黄和穆萧尹姚邵湛汪祁毛禹狄米贝明臧计成戴宋茅庞熊纪舒屈项祝董粱杜阮席季麻强贾路娄危江童颜郭梅盛林刁钟徐邱骆高夏蔡田胡凌霍万柯卢莫房缪干解应宗丁宣邓郁单杭洪包诸左石崔吉"                 "龚程邢滑裴陆荣翁荀羊甄家封芮储靳邴松井富乌焦巴弓牧隗山谷车侯伊宁仇祖武符刘景詹束龙叶幸司韶黎乔苍双闻莘劳逄姬冉宰桂牛寿通边燕冀尚农温庄晏瞿茹习鱼容向古戈终居衡步都耿满弘国文东殴沃曾关红游盖益桓公晋楚闫"
        # 百家姓全部姓氏
    ...
    @staticmethod
    def en_name(firstName_name, last_name):
        en_firstName_name = pinyin.get(firstName_name, format="strip", delimiter=" ").upper()
        en_last_name = pinyin.get(last_name, format="strip", delimiter=" ").upper()
        return en_firstName_name, en_last_name


if __name__ == '__main__':
    firstName_name, last_name = Name.random_name()
    print(firstName_name + last_name)
    print(firstName_name + last_name)
    en_firstName_name, en_last_name = Name.en_name(firstName_name, last_name)
    print(en_firstName_name, en_last_name)
